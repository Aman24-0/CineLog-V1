  ...
  <div onClick={props.onClick} class="group cursor-pointer animate-pop-in relative">
    <div class="aspect-[2/3] rounded-3xl overflow-hidden relative shadow-[0_8px_30px_rgb(0,0,0,0.5)] group-hover:-translate-y-2 transition-all duration-300 border border-white/10 group-hover:border-[var(--primary)]/50 bg-[#171921]">
      ...
      <div class="absolute bottom-0 left-0 w-full p-3 flex flex-col justify-end">
        ...
        <p class="text-[9px] text-gray-300 font-bold mb-2 flex items-center gap-1 drop-shadow-md whitespace-nowrap overflow-hidden">
        ...
        <div class="flex gap-1 overflow-hidden">
            <span class="text-[9px] px-1.5 py-0.5 rounded-md bg-black/60 border border-white/5 font-black text-[#f5c518] flex items-center gap-0.5 shrink-0 shadow-sm"><Icon name="star" class="text-[10px]" fill/> {props.movie.imdbRating || '-'}</span>
            <span class="text-[9px] px-1.5 py-0.5 rounded-md bg-black/60 border border-white/5 font-black text-red-500 flex items-center gap-0.5 shrink-0 shadow-sm">🍅 {props.movie.rtRating || '-'}</span>
            <span class="text-[9px] px-1.5 py-0.5 rounded-md bg-[var(--primary)]/20 border border-[var(--primary)]/20 font-black text-[var(--primary)] flex items-center gap-0.5 shrink-0 shadow-sm"><Icon name="person" class="text-[10px]" fill/> {props.movie.rating || '-'}</span>
        </div>
      </div>
    </div>
  </div>
...
function UpcomingView(props) {
  ...
  createEffect(() => {
    ...
    // FIX: Filter out Reality (10764) and Talk (10767) shows
    ...
    Promise.all([
        ...
        // FIX: Fetch TV Details to get next_episode_to_air date
...
// 9. DETAILS MODAL (Ultimate Restore)
        ...
        <div class="w-full max-w-xl bg-[#08090b]/80 backdrop-blur-3xl rounded-t-[2.5rem] sm:rounded-[2.5rem] overflow-hidden border border-white/10 relative max-h-[95vh] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] animate-pop-in flex flex-col" onClick={e=>e.stopPropagation()}>
          ...
          <div class="overflow-y-auto hide-scrollbar w-full">
              ...
              <div class="px-6 md:px-8 pb-28 -mt-16 relative z-10">
                ...
                {/* Horizontal Big Ratings Row */}
                <div class="flex gap-2 my-5 overflow-x-auto hide-scrollbar">
                    <div class="bg-black/40 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-xl flex items-center gap-1.5 shrink-0"><Icon name="star" fill class="text-[12px] text-[#f5c518]"/><span class="text-xs font-black text-white">{omdbData().imdb}</span><span class="text-[7px] font-black text-gray-500 uppercase tracking-widest ml-1">IMDb</span></div>
                    <div class="bg-black/40 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-xl flex items-center gap-1.5 shrink-0"><span class="text-xs font-black text-red-500">🍅</span><span class="text-xs font-black text-white">{omdbData().rt}</span><span class="text-[7px] font-black text-gray-500 uppercase tracking-widest ml-1">RT</span></div>
                    <div class="bg-[var(--primary)]/10 backdrop-blur-md border border-[var(--primary)]/20 px-3 py-1.5 rounded-xl flex items-center gap-1.5 shrink-0"><Icon name="person" fill class="text-[12px] text-[var(--primary)]"/><span class="text-xs font-black text-[var(--primary)]">{movie().rating ? `${movie().rating}/10` : '-'}</span><span class="text-[7px] font-black text-[var(--primary)] uppercase tracking-widest ml-1 opacity-70">Sage</span></div>
                </div>
