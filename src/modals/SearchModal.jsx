...
const clearRecentStorage = () => localStorage.removeItem(RECENT_KEY);
...
export function SearchModal(props) {
  ...
  createEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        setResults(filtered);
        if (filtered.length > 0) { saveRecent(query.trim()); setRecentSearches(loadRecent()); }
      } catch(e) {}
      setSearching(false);
    }, 350);
  ...
  const groupedResults = createMemo(() => {
    ...
    const flat = [];
    if (movies.length > 0) { flat.push({ type: 'header', label: 'Movies', icon: 'movie', count: movies.length }); movies.forEach(m => flat.push({ type: 'item', data: m })); }
    if (tv.length > 0)     { flat.push({ type: 'header', label: 'Series', icon: 'tv', count: tv.length });       tv.forEach(t => flat.push({ type: 'item', data: t })); }
    if (people.length > 0) { flat.push({ type: 'header', label: 'People', icon: 'person', count: people.length }); people.forEach(p => flat.push({ type: 'item', data: p })); }
    return flat;
  });
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      const pos = selectables.indexOf(highlightedIdx());
      setHighlightedIdx(selectables[pos < selectables.length - 1 ? pos + 1 : 0]);
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (!selectables.length) return;
      const pos = selectables.indexOf(highlightedIdx());
      setHighlightedIdx(selectables[pos > 0 ? pos - 1 : selectables.length - 1]);
    } else if (e.key === 'Enter') {
      if (highlightedIdx() >= 0) {
        e.preventDefault();
      ...
      if (q().length > 0) { setQ(''); if (inputRef) inputRef.focus(); }
      else props.onClose();
    }
  };

  const selectItem = (item) => {
    if (item.media_type === 'person') setPersonId(item.id);
    else { const isSaved = props.watchlist.some(w => String(w.id) === String(item.id)); if (!isSaved) props.openPreview(item); }
  };
  ...
  const handleClearRecent = () => { clearRecentStorage(); setRecentSearches([]); };
  ...
  const addMedia = async (m, e) => {
    ...
    if (props.watchlist.some(item => String(item.id) === String(m.id))) { props.showToast('Already in Vault! 🍿', 'info'); return; }
    ...
    try {
      ...
      props.showToast('Error adding to vault.', 'error');
    }
  };
  ...
  return (
    <div
      ...
      style="background: rgba(0,0,0,0.88); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px)"
      onClick={props.onClose}
      role="dialog"
      aria-modal="true"
      aria-label="Search CineLog"
    >
      <div
        class="animate-pop-spring w-full max-w-2xl mx-auto rounded-[1.75rem] overflow-hidden flex flex-col border shadow-2xl"
        style="background: #111; border-color: rgba(255,255,255,0.12); box-shadow: 0 24px 80px rgba(0,0,0,0.6), 0 0 40px var(--p-glow); max-height: 75vh"
        onClick={e => e.stopPropagation()}
        role="search"
      >
        {/* ── Search Input ── */}
        <div class="px-5 py-4 border-b flex gap-3 items-center shrink-0" style="border-color: rgba(255,255,255,0.07)">
          <Icon name="search" style="color: var(--p); font-size: 22px; flex-shrink: 0" aria-hidden="true" />
          <input
            ref={el => inputRef = el}
            autofocus
            value={q()}
          ...
          <Show when={searching()}>
            <div
              class="w-5 h-5 border-2 rounded-full animate-spin shrink-0"
              style="border-color: var(--p); border-top-color: transparent"
              role="status"
          ...
          <Show when={q().length > 0 && !searching()}>
            <button
              ...
              class="text-gray-500 hover:text-white active:scale-95 p-1.5 rounded-full hover:bg-white/5 shrink-0 transition-all"
              aria-label="Clear search query"
            >
              <Icon name="backspace" class="text-sm" aria-hidden="true" />
            </button>
          </Show>
          ...
          <button
            onClick={props.onClose}
            class="bg-white/10 hover:bg-white/20 active:scale-95 p-2 rounded-full ml-1 shrink-0 transition-all"
            aria-label="Close search"
          >
            <Icon name="close" class="text-white text-sm" aria-hidden="true" />
          </button>
        </div>

        {/* ── Content Area ── */}
        <div class="overflow-y-auto hide-scrollbar flex-1" role="listbox" aria-label="Search results">
          ...
          <Show when={!searching() && q().length < 2}>
            <div class="p-5 animate-fade-in">
              <Show when={recentSearches().length > 0}>
                <div class="mb-6">
                  <div class="flex items-center justify-between mb-3">
                    <span class="search-group-label flex items-center gap-2">
                      <Icon name="history" style="font-size: 13px; color: var(--muted)" aria-hidden="true" />
                      Recent
                    </span>
                    <button
                      onClick={handleClearRecent}
                      class="type-caption hover:text-white transition-colors px-2 py-1 rounded hover:bg-white/5 active:scale-95"
                  ...
                  <div class="flex flex-wrap gap-2" role="group" aria-label="Recent searches">
              ...
              <div>
                <span class="search-group-label flex items-center gap-2">
                  <Icon name="bolt" style="font-size: 13px; color: var(--p)" aria-hidden="true" />
                  Quick Search
                </span>
                <div class="flex flex-wrap gap-2 mt-3" role="group" aria-label="Suggested searches">
                  <For each={QUICK_SUGGESTIONS}>
                    {(term) => (
                      <button
                        onClick={() => handleSuggestionClick(term)}
                        class="recent-chip"
                        style="border-color: color-mix(in srgb, var(--p) 20%, transparent)"
                        aria-label={`Search for ${term}`}
                      >
                        {term}
                      </button>
              ...
              <Show when={recentSearches().length === 0}>
                <div class="text-center mt-10 opacity-30">
                  <Icon name="movie_filter" class="text-5xl mb-3" style="color: var(--muted)" aria-hidden="true" />
                  <p class="type-caption">Search movies, series &amp; actors</p>
                  <p class="type-caption mt-1" style="color: var(--dim)">Use ↑ ↓ to navigate, Enter to select, Esc to clear</p>
                </div>
              </Show>
            </div>
          </Show>
          ...
          {/* ── LOADING: Skeleton ── */}
          <Show when={searching()}>
            <div class="p-3 space-y-1" role="status" aria-label="Searching…">
              <For each={[
                ...
                {(s) => (
                  <div
                    class="flex gap-4 p-3 rounded-[1.5rem] border border-transparent"
                    aria-hidden="true"
                  >
                    <Show
                      when={s.poster}
                      fallback={<div class="w-12 h-12 rounded-full skeleton-bg shrink-0" />}
                    >
          ...
          <Show when={!searching() && results().length > 0}>
            <div class="p-3 stagger" role="list">
              <For each={groupedResults()}>
                {(entry, idx) => {
                  /* GROUP HEADER */
                  if (entry.type === 'header') {
                    return (
                      <div class="search-group-header animate-fade-in" role="presentation">
                        <Icon name={entry.icon} style="font-size: 14px; color: var(--p)" aria-hidden="true" />
                        <span class="search-group-label">{entry.label}</span>
                        <span class="search-group-count">{entry.count}</span>
                        <div class="flex-1 h-px" style="background: var(--border)" aria-hidden="true" />
                      </div>
                    );
                  }
                  ...
                  if (item.media_type === 'person') {
                    return (
                      <div
                        ...
                        onKeyDown={e => { if (e.key === 'Enter') setPersonId(item.id); }}
                        class={`flex items-center gap-4 p-3 rounded-[1.5rem] border cursor-pointer group animate-fade-up ${isHighlighted() ? 'search-highlight' : 'border-transparent hover:border-[color-mix(in_srgb,var(--p)_25%,transparent)] hover:bg-white/[0.04]'}`}
                        role="option"
                        tabIndex={0}
                        aria-selected={isHighlighted()}
                        aria-label={`${item.name} — ${item.known_for_department === 'Directing' ? 'Director' : 'Actor'}`}
                      >
                        <img
                          ...
                          class="poster-img img-loaded w-12 h-12 rounded-full object-cover border border-white/10 shrink-0"
                          onLoad={e => e.target.classList.add('img-loaded')}
                          alt={item.name}
                        />
                        <div class="flex-1 min-w-0">
                          <p class="type-metadata font-black text-gray-100 line-clamp-1">{item.name}</p>
                          <p class="type-caption mt-0.5" style="color: var(--p)">
                            {item.known_for_department === 'Directing' ? '[DIRECTOR]' : '[ACTOR]'}
                          </p>
                        </div>
                        <Icon name="chevron_right" class="text-gray-500 group-hover:text-[color:var(--p)] shrink-0 transition-colors" aria-hidden="true" />
                      </div>
                    );
                  }
                  ...
                  return (
                    <div
                      ...
                      onKeyDown={e => { if (e.key === 'Enter' && !isSaved()) props.openPreview(item); }}
                      class={`flex gap-4 p-3 rounded-[1.5rem] border group animate-fade-up ${isSaved() ? 'opacity-60' : 'cursor-pointer'} ${isHighlighted() ? 'search-highlight' : 'border-transparent hover:border-[color-mix(in_srgb,var(--p)_25%,transparent)] hover:bg-white/[0.04]'}`}
                      role="option"
                      tabIndex={isSaved() ? -1 : 0}
                      aria-selected={isHighlighted()}
                      aria-label={`${item.title || item.name}${isSaved() ? ' — already in vault' : ' — tap to preview'}`}
                      aria-disabled={isSaved()}
                    >
                      <Show
                        when={item.poster_path}
                        fallback={
                          <div class="w-14 h-20 bg-white/5 rounded-xl flex items-center justify-center border border-white/5 bg-[#171921] shrink-0" aria-hidden="true">
                      ...
                      <div class="flex flex-col justify-center flex-1 py-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 mt-1.5">
                          ...
                          <Show when={item.vote_average > 0}>
                            <span class="type-caption flex items-center gap-1 text-[#f5c518]" aria-label={`Rating: ${item.vote_average.toFixed(1)}`}>
                              <Icon name="star" fill class="text-[9px]" aria-hidden="true" />
                        ...
                        <Show
                          ...
                          fallback={
                            <button
                              ...
                              class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 active:scale-90 hover:bg-[var(--p)] hover:text-[#08090b] hover:border-[var(--p)] hover:[box-shadow:0_0_12px_var(--p-glow)] transition-all"
                              aria-label={`Add ${item.title || item.name} to Vault`}
                            >
                              <Icon name="add" class="text-xl" aria-hidden="true" />
                            </button>
                          }
                        >
                          <div
                          ...
                          >
                            <Icon name="check_circle" fill class="text-xl" style="color: var(--p)" aria-hidden="true" />
                          </div>
                        </Show>
                      </div>
                    </div>
          ...
          <Show when={!searching() && q().length >= 2 && results().length === 0}>
            <div class="text-center p-12 animate-fade-in empty-state" style="color: var(--muted)">
              <div class="empty-state-icon mb-2" aria-hidden="true">
                <Icon name="search_off" style="color: var(--muted); font-size: 36px" />
              </div>
              <p class="empty-state-title" style="font-size: 1.5rem">Nothing Found</p>
              <p class="empty-state-body mb-4">No results in this universe. Try a different title or person.</p>
              <Show when={didYouMean()}>
                <button
                  onClick={() => setQ(didYouMean())}
                  class="type-caption inline-flex items-center gap-1.5 px-4 py-2 rounded-full border active:scale-95 transition-all"
                  style="color: var(--p); border-color: color-mix(in srgb, var(--p) 30%, transparent); background: var(--p-dim)"
                  aria-label={`Search for ${didYouMean()} instead`}
                >
                  <Icon name="spellcheck" class="text-[12px]" aria-hidden="true" />
                  Did you mean <strong class="text-white ml-1">{didYouMean()}</strong>?
                </button>
              </Show>
              <div class="mt-6">
                <span class="search-group-label justify-center flex items-center gap-2 mb-3">
                  <Icon name="bolt" style="font-size: 12px; color: var(--p)" aria-hidden="true" /> Try Instead
                </span>
                <div class="flex flex-wrap gap-2 justify-center" role="group" aria-label="Suggested searches">
                  <For each={QUICK_SUGGESTIONS.slice(0, 6)}>
                    {(term) => (
                      <button
                        onClick={() => handleSuggestionClick(term)}
                        class="recent-chip"
