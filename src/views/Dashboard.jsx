  ...
  return (
    <div class="animate-fade-in pb-8 space-y-8">
          ...
          return (
            <div class="featured-hero" role="region" aria-label={`Featured: ${item().title || item().name}`}>
              ...
              <div class="absolute bottom-0 left-0 w-full p-4 lg:p-6 flex flex-col gap-2 z-10">
                {/* Title — clamp to 2 lines to prevent overflow */}
                <h2
                ...
                <div class="flex items-center gap-3 type-metadata text-gray-300 flex-wrap">
                  ...
                  <Show when={item().imdbRating || item().rating}>
                    <span
                      class="rating-pill"
                      aria-label={`IMDb: ${item().imdbRating || item().rating}`}
                    >
      ...
      <Show when={props.isGuest}>
        <div
          class="p-6 rounded-3xl border relative overflow-hidden animate-fade-up"
      ...
      <Show when={!props.isGuest && props.watchlist().length > 0}>
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger" role="region" aria-label="Vault statistics">
          ...
          <div
            ...
            onKeyDown={e => { if (e.key === 'Enter') { props.setActiveVaultStatus('all'); props.setView('watchlist'); } }}
            class="stat-card p-4 flex flex-col items-center justify-center cursor-pointer group animate-fade-up"
            role="button"
            tabIndex={0}
            aria-label={`${props.watchlist().length} total titles in vault`}
          >
            <Icon name="inventory_2" class="text-gray-600 group-hover:text-white mb-2 text-xl" style="transition: color 220ms ease-out" aria-hidden="true" />
          ...
          <div
            ...
            onKeyDown={e => { if (e.key === 'Enter') { props.setActiveVaultStatus('Watching'); props.setView('watchlist'); } }}
            class="stat-card p-4 flex flex-col items-center justify-center cursor-pointer group animate-fade-up"
            role="button"
            tabIndex={0}
            aria-label={`${watchingCount()} titles currently watching`}
          >
            <Icon name="play_circle" class="text-gray-600 group-hover:text-white mb-2 text-xl" style="transition: color 220ms ease-out" aria-hidden="true" />
          ...
          <div
            ...
            onKeyDown={e => { if (e.key === 'Enter') { props.setActiveVaultStatus('Planned'); props.setView('watchlist'); } }}
            class="stat-card p-4 flex flex-col items-center justify-center cursor-pointer group animate-fade-up"
            role="button"
            tabIndex={0}
            aria-label={`${plannedList().length} titles planned to watch`}
          >
            <Icon name="bookmark" class="text-gray-600 group-hover:text-[color:var(--p)] mb-2 text-xl" style="transition: color 220ms ease-out" aria-hidden="true" />
          ...
          <div
            ...
            onKeyDown={e => { if (e.key === 'Enter') { props.setActiveVaultStatus('Completed'); props.setView('watchlist'); } }}
            class="stat-card p-4 flex flex-col items-center justify-center cursor-pointer group animate-fade-up"
            role="button"
            tabIndex={0}
            aria-label={`${completedCount()} titles completed`}
          >
            <Icon name="task_alt" class="text-gray-600 group-hover:text-white mb-2 text-xl" style="transition: color 220ms ease-out" aria-hidden="true" />
          ...
          <div class="flex gap-4 overflow-x-auto hide-scrollbar pb-4" role="list">
            <For each={continueWatchingList()}>
              {(m) => {
                ...
                const progressLabel = m.media_type === 'tv'
                ...
                return (
                  <div
                    ...
                    onKeyDown={e => { if (e.key === 'Enter') props.openMovie('RESUME_' + m.id); }}
                  ...
                  >
                    <Show
                      when={bgImg}
                      fallback={
                        <div class="absolute inset-0 flex items-center justify-center skeleton-bg" aria-hidden="true">
